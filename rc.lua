...
local cpu_usage         = require("modules.widgets.cpu_usage")
local pulseaudio        = require("modules.libraries.backend.pulseaudio")
local playercontrol     = require("modules.libraries.backend.playercontrol")
local smart_run_cmd     = require("modules.libraries.backend.smart_run_cmd")
local make_panel_widget = require("modules.libraries.end-user.make_panel_widget")
...
screen.connect_signal("request::desktop_decoration", function(s)
					...
					{
						{
							...
							hover_color = beautiful.nord9,
							create_callback = function(self, c3, index, objects) --luacheck: no unused args
								self:connect_signal("mouse::enter", function()
									if self.bg ~= self.hover_color then
										self.backup     = self.bg
	...
	s.bar_widgets = {
		main_menu_launcher = function()
			...
			return main_menu_launcher
		end
	}

	s.widgets.volume_slider_slider = pulseaudio.widget.volume_slider {
		bar_shape           = gears.shape.rounded_bar,
		bar_height          = 3,
		bar_color           = beautiful.nord4,
		handle_color        = beautiful.nord8,
		handle_shape        = gears.shape.circle,
	...
	s.widgets.volume_slider_widget = wibox.widget {
		{
			{
				s.widgets.volume_slider_slider,
				left = 5,
				right = 5,
				widget = wibox.container.margin
			},
	...
	s.widgets.song_info_label = playercontrol.widget.title_artist_label()

	s.widgets.song_info_widget_progress_bar = wibox.widget {
		color = {
			type  = "linear",
			from  = { 0  , 0 },
			to    = { 0, 20 },
	...
	s.widgets.song_info_widget = wibox.widget {
		{
			{

				s.widgets.song_info_widget_progress_bar,
				{
					s.widgets.song_info_label,
					left = 5,
					right = 5,
					widget = wibox.container.margin
				},
	...
	s.widgets.left_wibar_clock = wibox.widget {
		nil,
		{
			format = "%H:%M",
			font = "Source Sans Pro, Bold 10",
			widget = wibox.widget.textclock,
		},
		nil,
		expand = "outside",
	...
	s.widgets.top_wibar = awful.wibar {
		...
		widget = {
			layout = wibox.layout.align.horizontal,
			{ -- Left widgets
			s.bar_widgets.main_menu_launcher(),
				layout = wibox.layout.fixed.horizontal,
				--s.widgets.taglist,
				s.widgets.promptbox,
			},
	...
	s.widgets.left_wibar = awful.wibar {
		...
		width = 45,
		widget = {
			{ -- Top widget_slider_bar_color
				s.widgets.taglist,
				layout = wibox.layout.fixed.vertical,
			},
			nil,
			{ -- Right widgets
				s.widgets.left_wibar_clock,
				s.widgets.layoutbox,
				--widget_wrapper(s.widgets.layoutbox),
				layout = wibox.layout.fixed.vertical,
			},
			layout = wibox.layout.align.vertical,
		}
