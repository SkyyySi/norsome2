-- This is a modified version of awful.menu included with awesome which
-- has an additional padding around each icon.
...
function menu.entry(parent, args) -- luacheck: no unused args
    ...
    if icon then
		...
		--local iconbox_widget = wibox.widget.imagebox()
        iconbox = wibox.widget { -- Here's the part that was altered for norsome/qrlinux.
			{
        ...
        if iconbox.widget:set_image(icon) then
